...
using Paises = Veritrade2017.Models.Paises;
	...
	public class HomeController : BaseController
	{
		private void GetAddressIP(ref string codPaisIp)
		{
		   var inci= new Incidencias();
			string xx = codPaisIp != null ? codPaisIp : "nulo";
			inci.SalvarIncidencia($"1.- codPais parametro:{xx}");

			var ws = new VeritradeAdmin.Seguridad();

			 xx = codPaisIp != null ? ws.ToString() : "nulo";
			inci.SalvarIncidencia($"2.- ws :{xx}");

#if DEBUG
			var direccionIp = Properties.Settings.Default.IP_Debug;
			xx = direccionIp != null ? direccionIp.ToString() : "nulo";
			inci.SalvarIncidencia($"3.- direccionIp debug :{xx}");
#else
			var direccionIp = Request.ServerVariables["REMOTE_ADDR"];
			//direccionIp = Request.ServerVariables["REMOTE_ADDR"];
			xx = direccionIp != null ? direccionIp.ToString() : "nulo";
			inci.SalvarIncidencia($"35 .- direccionIp REMOTE_ADDR  :{xx}");
#endif
		...
		{
			var inci = new Incidencias();
			string xx = culture != null ? culture.ToString() : "nulo";
			inci.SalvarIncidencia($"1 Index- culture  :{xx}");
			
			xx = url != null ? url.ToString() : "nulo";
			inci.SalvarIncidencia($"2 Index- url  :{xx}");


			string languajeBrowser = SetupLanguaje();

			xx = languajeBrowser != null ? languajeBrowser.ToString() : "nulo";
			inci.SalvarIncidencia($"2 Index- url  :{xx}");
			...
			if (idiomaT != "")
			{
				...
				xx = idiomaT != null ? idiomaT.ToString() : "nulo";
				inci.SalvarIncidencia($"3 Index- idiomaT  :{xx}");
				xx = codPaisIpt != null ? codPaisIpt.ToString() : "nulo";
				inci.SalvarIncidencia($"3 Index- codPaisIpt  :{xx}");
			}
			else
			{
				...
				xx = idioma != null ? idioma.ToString() : "nulo";
				inci.SalvarIncidencia($"4 Index- idioma  :{xx}");
				...
				xx = idioma != null ? idioma.ToString() : "nulo";
				inci.SalvarIncidencia($"5 Index- idioma  :{xx}");

				xx = codPaisIp != null ? codPaisIp.ToString() : "nulo";
				inci.SalvarIncidencia($"6 Index- codPaisIp  :{xx}");

				var currentCulture = CultureHelper.GetCurrentCulture();


				xx = currentCulture != null ? currentCulture.ToString() : "nulo";
				inci.SalvarIncidencia($"7 Index- currentCulture  :{xx}");
			...
			var ambiente = Properties.Settings.Default.Ambiente.ToUpper() ?? "DESARROLLO";

			if (ambiente == "DESARROLLO")
			{
				if (Variables.DevelopInitialPageSW == 0)
				{

					Variables.DevelopInitialPageSW += 1;
					return RedirectToRoute("PasswordBypass");
		...
		private string SetupLanguaje()
		{
			...
			try
			{
				CultureInfo cultureInfo = new CultureInfo(userLanguages[0]);
