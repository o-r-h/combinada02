...
namespace Veritrade2018.Helpers
{
    ...
    public class VarGeneral
    {
        ...
        public  List<VarValues> Values
        {
            get
            {
                var x = Properties.Settings.Default.TableVarGeneral_InDev;
                var sql = string.Format("select * from {0}dbo.VariableGeneral Where Estado=1 Order By iOrden",
                     (x ? Properties.Settings.Default.Server_Dev+".VeritradeBusiness." : string.Empty ) );
                ...
                _Values = new List<VarValues>();
                    using (var db = new ConexProvider().Open)
                    {
                        db.Query<VarValues>(sql,commandTimeout:0)
        ...
        public Dictionary<string, VarValues> ValuesDict
        {
            get
            {
                var dic = new Dictionary<string, VarValues>();
                try
                {
                    dic = Values?.ToDictionary(x => x.IdVariable, y => y);
                } catch (Exception)
                {
                    dic = Values?.ToDictionary(x => x.IdVariable, y => y);
                }
    ...
    public class TabMisBusquedas : VarFiltros
    {
        ...
        // Ruben 202210
        private readonly string[] _paisesConInfoComplementaria = new[] { "BO", "BR", "CA", "CN", "CR", "CRS", "GT", "HN", "IN", "MX", "RD", "SV", "UE","US"};
        ...
        public bool ExistePartida,
            ...
            ExisteInfoTabla, ExisteInfoComplementario;
        ...
        public void SetTabMisBusquedas()
        {
            ...
            ExisteInfoComplementario = /*Properties.Settings.Default.TableVarGeneral_InDev &&*/ _paisesConInfoComplementaria.Contains(CodPais);//true;
