...
namespace Veritrade2018.Controllers
{
    public class ConsultaController : BaseController
    {
        ...
        public JsonResult rdbtnTipoOpeChange(string selectedTipoOpe, string codPais, string idiomaCulture)
        {
            ...
            var consultaForm = GetConsultaForm(auxCodPais, selectedTipoOpe);
        ...
        public JsonResult cboPaisSelectedIndexChanged(string tipoOpe, string codPais, string idiomaCulture)
        {
            ...
            var consultaForm = GetConsultaForm(codPais, tipoOpe);
            Restablecer();
            return Json(new
            {
                htmlFormSection = RenderViewToString(this.ControllerContext, "Consulta/_FormSection", consultaForm),
        ...
        [HttpPost]
        public JsonResult Buscar(string fechaDesde, string fechasHasta, string TipoOpe, string CodPais)
        {
            return BuscarPorFiltro(fechaDesde, fechasHasta, TipoOpe, CodPais);
        }

        private JsonResult BuscarPorFiltro(string fechaIni, string fechaFin, string TipoOpe, string CodPais)
        {
            Session["CodPais"] = CodPais;
            ...
            #region Filtros
            Session["SqlFiltro"] = GeneraSqlFiltro(fechaIni, fechaFin, dua);
            ...
            else if (cantReg > CantRegMax && !flagPalabras)
            {
                ...
                Session["SqlFiltro"] = GeneraSqlFiltro(fechaIni, fechaFin, dua);
                drTotales = CalculaTotales(Session["SqlFiltro"].ToString(), CIFTot, CodPais, tabla);
                                                ...
                                                string tabla, string dua, string valueCIFTot, CultureInfo specificCulture)
        {
            ...
            if (TipoOpe == "I")
            {
                if (Consulta.ExisteVariable(CodPais, TipoOpe, "IdImportador"))
                {
                    numRows = 0;
                ...
                if (Consulta.ExisteVariable(CodPais, TipoOpe, "IdProveedor"))
                {
                    numRows = 0;
                ...
                if (Consulta.ExisteVariable(CodPais, TipoOpe, "IdPaisOrigen"))
                {
                    numRows = 0;
                    dataTable = CargaFiltro(CodPais, CIFTot, SessionSqlFiltro, tabla, dua, "PaisOrigen", -1);
                    numRows = dataTable.Rows.Count;
                    if (numRows > 0)
                    {
                        json.Add(new
                        {
                            ...
                            tabDataNumRows = numRows,
                            tabDataList = GeneraJsonDataPaisOrigen(dataTable, CIFTot, valueCIFTot, specificCulture),
                            tabDataPie = GeneraJsonDataPie("PaisOrigen", dataTable, CIFTot, valueCIFTot, specificCulture)
                        });
                    }
                    else
                    {
            ...
            //ExisteViaTransp
            if (CodPais != "BO" && CodPais != "PY" && CodPais != "US")
            {
                numRows = 0;
            ...
            //ExisteAduana
            if (CodPais != "CN" && CodPais != "IN" && CodPais != "PY" && CodPais != "US")
            {
        ...
        public JsonResult SearchByFilterPostFilterUpdate(string fechaDesde, string fechasHasta, string TipoOpe, string CodPais)
        {
            var specificCulture = Session["culture"].ToString().Equals("es") ? CultureInfo.CreateSpecificCulture("es-pe") : CultureInfo.CreateSpecificCulture("en-us");
            ...
            Session["SqlFiltro"] = GeneraSqlFiltro(fechaDesde, fechasHasta, dua);
        ...
        public string GeneraSqlFiltro(string mesIni, string mesFin, string DUA)
        {
            var sql = "";
            var codPais = Session["CodPais"].ToString();
            ...
            {
                sql += "and (";
                var importadores = Session["hdfImportadorB"].ToString().Split('|');
            ...
            {
                sql += "and (";
                var exportadores = Session["hdfExportadorB"].ToString().Split('|');
            ...
            if (Session["hdfProveedorB"] != null)
            {
                ...
                var palabras = Session["hdfProveedorB"].ToString().Split(' ');
                foreach (var palabra in palabras)
